...
def check_claude3_availability(region):
    try:
        ...
        claude3_models = [
            model for model in available_models if claude3_pattern.match(model)
        ]
        ...
        if e.response["Error"]["Code"] == "AccessDeniedException":
            print(
                "Access denied. Check if your credentials have the necessary permissions."
            )
        else:
...
def main():
    ...
    if has_privileges_to_invoke_bedrock(region):
        if check_claude3_availability(region):
            ...
            print(
                "\033[92mPre-flight validation passed. You can proceed with invoking AWS Bedrock (Claude3).\033[0m"
            )
        else:
            # ANSI escape sequence for red color
            print(
                "\033[91mPre-flight validation failed. Claude3 models are not available in the specified region.\033[0m"
            )
    else:
        # ANSI escape sequence for red color
        print(
            "\033[91mPre-flight validation failed. User does not have privileges to invoke AWS Bedrock (Claude3).\033[0m"
        )
